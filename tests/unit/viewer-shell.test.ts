...
vi.mock("../../lib/sensors/location", async () => {
  const actual = await vi.importActual<
    typeof import("../../lib/sensors/location")
...
vi.mock("../../lib/sensors/orientation", async () => {
  const actual = await vi.importActual<
    typeof import("../../lib/sensors/orientation")
...
vi.mock("../../lib/projection/camera", async () => {
  const actual = await vi.importActual<
    typeof import("../../lib/projection/camera")
...
vi.mock("../../lib/viewer/motion", async () => {
  const actual = await vi.importActual<
    typeof import("../../lib/viewer/motion")
  >("../../lib/viewer/motion");
...
describe("ViewerShell startup gating", () => {
  ...
  it("blocks live startup behind a secure context requirement", async () => {
    ...
    const startArButton = Array.from(container.querySelectorAll("button")).find(
      (button) => button.textContent?.includes("Start AR"),
    );

    expect(container.textContent).toContain(
      "Live AR requires a secure context.",
    );
  ...
  it("keeps location and orientation startup live in verified non-camera fallback", async () => {
    ...
    expect(
      mockSubscribeToOrientationPose.mock.calls.length,
  ...
  it("keeps location and camera startup live in verified manual-pan fallback", async () => {
    ...
    expect(mockFetchAircraftSnapshot).toHaveBeenCalledTimes(1);
    expect(
      mockRequestRearCameraStream.mock.calls.length,
    ).toBeGreaterThanOrEqual(1);
    expect(mockSubscribeToOrientationPose).not.toHaveBeenCalled();
  });
  ...
  it("reopens the live camera when the picker switches back to auto rear camera", async () => {
    ...
    expect(mockRequestRearCameraStream).toHaveBeenCalledTimes(1);
    expect(latestSettingsProps()?.onSelectedCameraDeviceChange).toBeTypeOf(
      "function",
    );
  ...
  it("collapses mobile chrome behind the bottom trigger and restores it when opened", async () => {
    ...
    expect(mobileTrigger?.getAttribute("aria-controls")).toBe(
      "mobile-viewer-overlay",
    );
    expect(mobileTrigger?.getAttribute("aria-expanded")).toBe("false");
    expect(
      container.querySelector('[data-testid="mobile-viewer-overlay"]'),
    ).toBeNull();
    expect(
      container.querySelector('[data-testid="mobile-viewer-header"]'),
    ).toBeNull();
    ...
    expect(
      mobileOverlay?.querySelector('[data-testid="settings-sheet"]'),
    ).not.toBeNull();
    ...
    const closeButton = Array.from(
      mobileOverlay!.querySelectorAll("button"),
  ...
  it("keeps blocked-state actions reachable inside the expanded mobile overlay", async () => {
    ...
    expect(
      container.querySelector('[data-testid="mobile-viewer-overlay-shell"]'),
    ).toBeNull();
    expect(
      mobileOverlay?.querySelector('[data-testid="settings-sheet"]'),
    ).not.toBeNull();
  ...
  it("shows a mobile marker-scale slider above the quick actions and persists live changes", async () => {
    await renderViewer({
      entry: "live",
      location: "unknown",
      camera: "unknown",
    ...
    const buttonRow = quickActions?.lastElementChild as HTMLElement | null;

    expect(quickActions).not.toBeNull();
    expect(quickActions?.textContent).toContain("Marker scale");
    expect(quickActions?.textContent).toContain("1.0");
    expect(slider).not.toBeNull();
    expect(slider?.value).toBe("1");
    expect(
      quickActions?.firstElementChild?.querySelector(
        '[data-testid="mobile-marker-scale-slider"]',
      ),
    ).toBe(slider);
    expect(
      buttonRow?.querySelector('[data-testid="mobile-viewer-overlay-trigger"]'),
    ).not.toBeNull();

    await act(async () => {
      setInputValue(slider!, "2.4");
    });

    expect(slider?.value).toBe("2.4");
    expect(quickActions?.textContent).toContain("2.4");
  ...
  it("surfaces live-panel blocker copy and a disabled start action before the first motion sample", async () => {
    ...
    expect(
      container.querySelector('[data-testid="alignment-instructions-panel"]'),
    ).not.toBeNull();
    expect(
      container.querySelector('[data-testid="alignment-focus-instruction"]'),
    ).toBeNull();
    expect(
      container.querySelector('[data-testid="alignment-crosshair-button"]'),
    ).toBeNull();
    expect(
      container.querySelector('[data-testid="alignment-start-action"]'),
    ).not.toBeNull();
    expect(
      (
        container.querySelector(
  ...
  it("keeps Start alignment visible if the mobile viewer overlay is reopened while alignment is open", async () => {
    ...
    await act(async () => {
      openViewerButton!.dispatchEvent(
  ...
  it("routes the mobile align action through the panel before entering alignment focus", async () => {
    mockSubscribeToOrientationPose.mockImplementationOnce(
      (onPose: (state: unknown) => void) => {
        onPose({
          pose: {
            yawDeg: 0,
            pitchDeg: 0,
    ...
    const calibrationCallsBefore =
      SENSOR_CONTROLLER.setCalibration.mock.calls.length;
    ...
    expect(
      container.querySelector('[data-testid="mobile-viewer-overlay-trigger"]'),
    ).not.toBeNull();
    expect(
    ...
    expect(SENSOR_CONTROLLER.setCalibration.mock.calls.length).toBe(
      calibrationCallsBefore,
    );
    expect(
      container.querySelector('[data-testid="mobile-align-action"]'),
    ).not.toBeNull();
    expect(
      container.querySelector('[data-testid="alignment-instructions-panel"]'),
    ).not.toBeNull();
    ...
    await act(async () => {
      startAlignmentButton!.dispatchEvent(
        new MouseEvent("click", { bubbles: true }),
      );
    });
    await flushEffects();
  ...
  it("exposes on-view manual nudges and gated reset controls in the alignment panel", async () => {
    ...
    const resetButton = Array.from(container.querySelectorAll("button")).find(
      (button) => button.textContent?.includes("Reset calibration"),
    ) as HTMLButtonElement | undefined;
    const nudgeRightButton = Array.from(
      container.querySelectorAll("button"),
    ).find((button) => button.textContent?.includes("Nudge right")) as
      | HTMLButtonElement
      | undefined;
    ...
    await act(async () => {
      nudgeRightButton!.dispatchEvent(
        new MouseEvent("click", { bubbles: true }),
      );
    });
    await flushEffects();
  ...
  it("shows the secure-context failure screen when live AR is unavailable on this origin", async () => {
    ...
    expect(container.textContent).toContain(
      "Live AR requires HTTPS or `localhost`",
    );
  ...
  it("makes the compact alignment panel internally scrollable on mobile", async () => {
    ...
    const alignmentPanel =
      alignmentPanels.find((panel) =>
        panel.className.includes("overflow-y-auto"),
      ) ?? null;
    const startAlignmentButton = container.querySelector(
      '[data-testid="alignment-start-action"]',
    ) as HTMLButtonElement | null;
  ...
  it("surfaces motion recovery guidance and retries orientation permission", async () => {
    ...
    const enableMotionButton = Array.from(
      container.querySelectorAll("button"),
    ).find((button) => button.textContent?.includes("Enable motion"));

    expect(enableMotionButton).toBeDefined();

    await act(async () => {
      enableMotionButton!.dispatchEvent(
        new MouseEvent("click", { bubbles: true }),
      );
    });
    await flushEffects();
  ...
  it("surfaces the relative-sensor warning state and calibration target guidance", async () => {
    ...
    expect(container.textContent).toContain(
      "Relative sensor mode needs alignment.",
    );
    expect(container.textContent).toMatch(
  ...
  it("falls back to the coarse scene cadence when reduced motion is preferred", async () => {
    ...
    try {
      ...
      expect(getLatestSceneTimeMs()).toBeGreaterThanOrEqual(
        initialSceneTimeMs + 1_000,
      );
    } finally {
      restoreAnimationFrame();
    }
  ...
  it("keeps demo scene time monotonic when reduced motion turns on mid-session", async () => {
    ...
    try {
      ...
      expect(getLatestSceneTimeMs()).toBeGreaterThanOrEqual(
        sceneTimeBeforePreferenceChange,
      );
    } finally {
      restoreAnimationFrame();
    }
  ...
  it("retains previous and current aircraft snapshots across live polls", async () => {
    ...
    try {
      ...
      const firstCall = mockResolveAircraftMotionObjects.mock.calls.at(
        -1,
      )?.[0] as
        | {
            snapshot?: unknown;
      ...
      const secondCall = mockResolveAircraftMotionObjects.mock.calls.at(
        -1,
      )?.[0] as
        | {
            snapshot?: unknown;
  ...
  it("keeps the last aircraft snapshot active when a live refresh fails", async () => {
    ...
    try {
      ...
      const latestCall = mockResolveAircraftMotionObjects.mock.calls.at(
        -1,
      )?.[0] as { snapshot?: unknown; previousSnapshot?: unknown } | undefined;
  ...
  it("uses requestAnimationFrame as the render-loop fallback when video-frame callbacks are unavailable", async () => {
    ...
    try {
      ...
      const frameTokenBefore = Number(
        stage?.getAttribute("data-frame-token") ?? "0",
      );
      ...
      expect(
        Number(stage?.getAttribute("data-frame-token") ?? "0"),
      ).toBeGreaterThan(frameTokenBefore);
    } finally {
      Object.defineProperty(window, "requestAnimationFrame", {
        configurable: true,
        writable: true,
  ...
  it("syncs fine-adjust and reset calibration actions into persisted viewer settings", async () => {
    await renderViewer({
      entry: "live",
      location: "granted",
      camera: "granted",
    ...
    expect(latestSettingsProps()?.onFineAdjustCalibration).toBeTypeOf(
      "function",
    );
    ...
    await act(async () => {
      latestSettingsProps()?.onFineAdjustCalibration?.({
        axis: "yaw",
        deltaDeg: 0.75,
      });
    });
  ...
  it("wires live-panel fine-adjust and reset controls into the existing calibration path", async () => {
    ...
    const nudgeLeftButton = Array.from(
      container.querySelectorAll("button"),
    ).find((button) => button.textContent?.includes("Nudge left")) as
      | HTMLButtonElement
      | undefined;
    ...
    const resetCalibrationButton = Array.from(
      container.querySelectorAll("button"),
    ).find(
      (button) =>
    ...
    await act(async () => {
      resetCalibrationButton?.dispatchEvent(
        new MouseEvent("click", { bubbles: true }),
      );
    });
    await flushEffects();
  ...
  it("pushes fine-adjust and reset calibration changes into storage and the live sensor controller", async () => {
    mockSubscribeToOrientationPose.mockImplementation(
      (onPose: (state: unknown) => void) => {
        onPose({
          pose: {
            yawDeg: 0,
    ...
    expect(readViewerSettings().poseCalibration.calibrated).toBe(true);
    expect(readViewerSettings().poseCalibration.offsetQuaternion).not.toEqual([
      0, 0, 0, 1,
    ]);
  ...
  it("preserves the desktop viewer header and desktop content composition", async () => {
    ...
    expect(
      desktopHeader?.querySelector('[data-testid="settings-sheet"]'),
    ).not.toBeNull();
...
function getLatestSceneTimeMs() {
  const latestCall = mockResolveSatelliteMotionObjects.mock.calls.at(
    -1,
  )?.[0] as { timeMs?: number } | undefined;
