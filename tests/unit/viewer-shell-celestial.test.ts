...
describe("ViewerShell celestial behavior", () => {
  ...
  it("falls back to day or night defaults when neither Sun nor Moon is visible", async () => {
    ...
    const initialNightSettingsProps = mockSettingsSheetProps.mock
      .calls[0]?.[0] as
      | {
          alignmentTargetPreference?: "sun" | "moon";
        }
  ...
  it("falls back from suppressed sun to moon, then brightest planet, then brightest star", async () => {
    ...
    await act(async () => {
      latestSettingsPropsAfterPlanetFallback()?.onAlignmentTargetPreferenceChange?.(
        "moon",
      );
    });
  ...
  it("renders target-aware fallback instructions in the live alignment panel", async () => {
    ...
    expect(container.textContent).not.toContain(
      "Choose Sun or Moon as your preferred target.",
    );
    expect(
  ...
  it("defaults to center-only overlay copy for the center-locked object", async () => {
    ...
    expect(
      container.querySelector('[data-testid="center-lock-chip"]')?.textContent,
    ).toContain("Sun");
    expect(
      container.querySelectorAll('[data-testid="sky-object-label"]'),
    ).toHaveLength(0);
    expect(
      container.querySelector('[data-testid="sky-object-top-list"]'),
    ).toBeNull();
  });

  it("lets dim stars render at 1px while keeping brighter objects larger", async () => {
    mockNormalizeCelestialObjects.mockReturnValue({
      sunAltitudeDeg: -12,
      objects: [
        {
    ...
    expect(getMarkerVisualSizePx(sunMarker!)).toBeGreaterThan(
      getMarkerVisualSizePx(dimStarMarker!),
    );
    expect(getMarkerVisualSizePx(dimStarMarker!)).toBe(1);
  });

  it("preserves the existing scale-1 size for representative non-dim stars", async () => {
    mockNormalizeCelestialObjects.mockReturnValue({
      sunAltitudeDeg: -12,
      objects: [
        {
    ...
    const commonStarMarker = container.querySelector(
      '[data-testid="sky-object-marker"][data-object-id="star-common"]',
    ) as HTMLButtonElement | null;

    expect(commonStarMarker).not.toBeNull();
    expect(getMarkerVisualSizePx(commonStarMarker!)).toBe(8);
  });

  it("scales marker sizes from their scale-1 result and keeps non-star floors intact", async () => {
    ...
    const farSatelliteMarker = container.querySelector(
      '[data-testid="sky-object-marker"][data-object-id="satellite-floor"]',
    ) as HTMLButtonElement | null;

    expect(dimStarMarker).not.toBeNull();
    expect(farSatelliteMarker).not.toBeNull();
    expect(getMarkerVisualSizePx(dimStarMarker!)).toBe(4);
    expect(getMarkerVisualSizePx(farSatelliteMarker!)).toBe(24);
  });
  ...
  it("renders stable nearby labels when on-object mode is enabled", async () => {
    ...
    expect(labels.some((label) => label.textContent?.includes("Sun"))).toBe(
      true,
    );
    expect(
      container.querySelector('[data-testid="center-lock-chip"]'),
    ).toBeNull();
  });
  ...
  it("keeps the bottom dock on the centered object after another label is tapped", async () => {
    ...
    const venusButton = Array.from(container.querySelectorAll("button")).find(
      (button) => button.textContent?.includes("Venus"),
    );
  ...
  it("renders a low-quality motion vector for a moving object as scene time advances", async () => {
    ...
    expect(
      container.querySelector('[data-testid="motion-affordance-vector"]'),
    ).toBeNull();
    expect(
      container.querySelector('[data-testid="motion-affordance-trail"]'),
    ).toBeNull();
  ...
  it("lets a daylit focus-only planet reach the centered label path", async () => {
    ...
    const marsButton = Array.from(container.querySelectorAll("button")).find(
      (button) => button.textContent?.includes("Mars"),
    );
  ...
  it("keeps stale satellite treatment additive for the ISS badge state", async () => {
    ...
    expect(
      container.querySelector(
        '[data-testid="object-badge"][data-badge-id="motion-stale"]',
      ),
    ).not.toBeNull();
    expect(
      container.querySelector(
        '[data-testid="object-badge"][data-badge-id="iss"]',
      ),
    ).not.toBeNull();
  });
  ...
  it("hard-fails into demo mode when the live astronomy pipeline throws", async () => {
    ...
    expect(mockRouterReplace).toHaveBeenCalledWith(
      expect.stringMatching(/entry=demo/),
    );
  ...
  it("keeps aircraft markers visible when satellite propagation throws", async () => {
    ...
    expect(
      container.querySelector(
        '[data-testid="sky-object-marker"][data-object-id="icao24-alpha1"]',
      ),
    ).not.toBeNull();
