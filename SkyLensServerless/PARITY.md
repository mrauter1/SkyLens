...
| Scope deep-star dataset delivery | fork-owned static `/data/scope/v1` assets | `lib/scope/catalog.ts` stays local-only by default, optionally prefers CDN/R2 assets when both `NEXT_PUBLIC_SKYLENS_SCOPE_REMOTE_ENABLED` and `NEXT_PUBLIC_SKYLENS_SCOPE_REMOTE_BASE_URL` are explicitly configured, and falls back to the committed `/data/scope/v1` dataset on disable or fetch failure | `tests/unit/config-contract.test.ts`, `tests/unit/scope-runtime.test.ts`, `tests/unit/scope-data-build.integration.test.ts` |
...
| Live AR permissions-policy header | host/runtime config | `next.config.ts` keeps the same `Permissions-Policy` header during local Next runtime use, and `public/_headers` preserves the required `camera`, `geolocation`, `accelerometer`, `gyroscope`, and `magnetometer` contract in exported deployments | `tests/unit/next-config.test.ts`, `tests/e2e/embed.spec.ts`, `npm run build` |
| Embedded viewer delegation contract | embedded route + iframe allow list | `app/embed-validation/page.tsx` keeps the iframe `allow` contract for `camera`, `geolocation`, `accelerometer`, `gyroscope`, and `magnetometer` while rendering the fork-local viewer | `tests/e2e/embed.spec.ts` |
...
- Scope deep-star assets remain available offline from the committed `/data/scope/v1` fallback dataset. Deployments can opt into remote CDN/R2 delivery by setting `NEXT_PUBLIC_SKYLENS_SCOPE_REMOTE_ENABLED=true` together with `NEXT_PUBLIC_SKYLENS_SCOPE_REMOTE_BASE_URL=https://.../scope/v1`; the client will still fall back locally if the remote path is unavailable.
