...
- IMP-015 `non-blocking`
  - Reference: `SkyLensServerless/components/viewer/viewer-shell.tsx`, `SkyLensServerless/tests/unit/viewer-shell-resolvers.test.ts`, `SkyLensServerless/tests/unit/viewer-shell.test.ts`, `.autoloop/tasks/skylensserverless-ux-implementation/implement/phases/finalize-ux-contract/implementation_notes.md`
  - Current reviewer reruns in the authoritative package context confirm the scoped `viewer-shell` review fixes are landed and validated. The live workspace now keeps motion warnings visible as a compact notice without reintroducing stacked primary banners, aligns mobile restore behavior without relying on ambient `document.activeElement`, rejects hidden/non-focusable restore targets in both restore and trap paths, and records matching implementation notes. `./node_modules/.bin/vitest run tests/unit/settings-sheet.test.tsx tests/unit/alignment-tutorial.test.ts tests/unit/viewer-shell-resolvers.test.ts` passed with `20` tests, `./node_modules/.bin/vitest run tests/unit/viewer-shell.test.ts` passed with `70` tests, and `./node_modules/.bin/playwright test tests/e2e/demo.spec.ts tests/e2e/permissions.spec.ts tests/e2e/landing.spec.ts --project=chromium` passed all `14` Chromium specs after package-local browser/runtime bootstrap.
  - Minimal fix direction: none; no new blocking findings in the current workspace state.
