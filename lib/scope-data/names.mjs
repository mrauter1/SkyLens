...
const HYG_PROPER_NAME_HEADER = ['hip', 'name']
...
export function parseHygProperNamesCsv(csvText) {
  const normalizedText = String(csvText ?? '').replace(/^\uFEFF/, '')
  const lines = normalizedText
    .split(/\r?\n/)
    .map((line) => line.trimEnd())
  ...
  if (
    header.length !== HYG_PROPER_NAME_HEADER.length ||
    header.some((value, index) => value !== HYG_PROPER_NAME_HEADER[index])
  ) {
    throw new Error(
      `HYG proper-name CSV header must be exactly "${HYG_PROPER_NAME_HEADER.join(',')}"`
    )
  }
  ...
  return lines.slice(1).map((line, index) => {
    ...
    if (row.length !== 2) {
      throw new Error(`Invalid HYG proper-name CSV row at line ${index + 2}`)
    }

    const hipId = Number(String(row[0] ?? '').trim())

    if (!Number.isInteger(hipId) || hipId <= 0) {
      throw new Error(`Invalid HYG proper-name HIP at line ${index + 2}`)
    }

    return {
      hipId,
      name: normalizeScopeDisplayName(row[1]),
      lineNumber: index + 2,
    }
  })
}

export function buildHygProperNameMap(rows) {
  const hipNameMap = new Map()

  for (const row of rows) {
    if (!Number.isInteger(row.hipId) || row.hipId <= 0) {
      continue
    }
    ...
    const existingName = hipNameMap.get(row.hipId)

    if (existingName && existingName !== row.name) {
      throw new Error(
        `Conflicting normalized HYG names for HIP ${row.hipId}: "${existingName}" vs "${row.name}"`
      )
    }

    hipNameMap.set(row.hipId, row.name)
  }
...
export function resolveScopeDisplayName({
  ...
  if (Number.isInteger(row.hipId)) {
    const hygName = hygProperNameMap.get(row.hipId)
